...
std::pair<bool, int> SecondaryBTree::getArticle(SecondaryBTreeDataMap& key, Article_t* article, FILE* indexFile)
{
  ...
  while (!currentPointer.first) {
    ...
    unsigned short toSeek =
    currentPointer.second >= reinterpretation->node.count ?
    reinterpretation->node.blockPointers[reinterpretation->node.count]: (key< reinterpretation->node.keys[currentPointer.second] ?reinterpretation->node.blockPointers[currentPointer.second]: reinterpretation->node.blockPointers[currentPointer.second + 1]);
